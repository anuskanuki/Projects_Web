...
$(document).ready(function () {
    ...
    $.each(data, function (key, value) {
        var Nomelogradouro = value.logradouro;
        $("#SelectDaNuki").append
            ("<option value='" + Nomelogradouro + "'>" + Nomelogradouro + "</option>");
        //var templateContent = "";
        //templateContent += "{";
        ...
        //});
        //templateContent += "}, ";
        //$('#divContent').append("<textarea>" + templateContent + "</textarea>");
    });
    //});

    //se tivesse a API ainda:
    //$('#SelectDaNuki').change(function () {
    //    $.get("https://viacep.com.br/ws/RS/Porto%20Alegre/all/json/", function (data) {
    //   // $.get(data,function (data) {
    //        $.get($('#SelectDaNuki').val(), function (data) {
    ...
    $("#SelectDaNuki").change(function () {
        /*$.get($('#logradouro').val(), function (data) {
        });*/
        var newdata = FindByLog($('#SelectDaNuki').val());



        $('#cep').val(newdata.cep);
        $('#bairro').val(newdata.bairro);
        $('#cidade').val(newdata.localidade);
        $('#estado').val(newdata.uf);
        if (newdata.complemento == "") {
            $('#unidade').val("não informado");
        } else {
            $('#unidade').val(newdata.unidade);
        }
        if (newdata.complemento == "") {
            $('#complemento').val("não informado");
        } else {
            $('#complemento').val(newdata.complemento);
        }
        if (newdata.complemento == "") {
            $('#ibge').val("não informado");
        } else {
            $('#ibge').val(newdata.ibge);
        }
        if (newdata.complemento == "") {
            $('#gia').val("não informado");
        } else {
            $('#gia').val(newdata.gia);
        }
    ...
    function FindByLog(logradouro) {
        var retorno;

        $.each(data, function (key, propert) {
            if (logradouro == propert.logradouro)
                retorno = propert;
        });
